...
export function createTools(config: NwnMcpConfig) {
  ...
  async function compileAll(input:z.infer<typeof schemas.compileAll>) { let files: string[] = input.changedOnly ? (await git.status()).modified.filter((f:string)=>/\.nss$/i.test(f)) : await fg([`${config.paths.scripts}/**/*.nss`],{cwd:root}); const results: Array<Awaited<ReturnType<typeof compile>>> = []; for (const f of files) results.push(await compile({scriptPath:f})); const diagnostics = results.flatMap(r=>r.diagnostics); return { success: results.every(r=>r.success), compiled: results.length, failed: results.filter(r=>!r.success).length, diagnostics }; }
