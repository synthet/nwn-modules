...
async function fixture(): Promise<{root:string; tools:ReturnType<typeof createTools>}> {
  ...
  const cfg: NwnMcpConfig = { workspaceRoot:'.', resolvedWorkspaceRoot:root, tools:{nasher:{command:'nasher'}, nwscriptCompiler:{command:'nwnsc'}, auroraToolset:{command:'toolset'}, nwnx:{root:'./external/nwnx', serverCommand:'nwserver'}}, paths:{moduleSource:'src', scripts:'src/scripts', docs:'docs', hakSource:'hak/source', dist:'build/dist', build:'build'}, permissions:{allowInstallToNwnUserDir:false, allowStartServer:false, allowAuroraLaunch:false} };
  return {root, tools:createTools(cfg)};
}
describe('tools', () => {
